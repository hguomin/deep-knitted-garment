...
def draw_text(input_image, content):
    ...
    black = (0, 0, 255) #np.array([0, 0, 255])
    margin = 15
    start_x = 5
    start_y = margin
...
# Guomin: render the garment on image
def visualize_reconstructed_garment(img, img_size, gt_kp, vertices, faces, pred_kp, camera, renderer, color='red', focal_length=1000):
    """Overlays gt_kp and pred_kp on img.
    Draws vert with text.
    Renderer is an instance of SMPLRenderer.
    """
    ...
    res = img.shape[1]
    camera_t = np.array([camera[1], camera[2], 2*focal_length/(res * camera[0] +1e-9)])
    rend_img = renderer.render(vertices, faces=faces, camera_t=camera_t,
                               img=img, use_bg=True,
                               focal_length=focal_length,
                               body_color=color)
    rend_img = draw_text(rend_img, debug_text)
...
class Renderer(object):
    ...
    def __init__(self, width=800, height=600, near=0.5, far=1000, faces=None):
        self.colors = {'pink': [.9, .7, .7], 'light_blue': [0.65098039, 0.74117647, 0.85882353], 'red': [.9, 0.0, 0.0] }
        self.width = width
        self.height = height
        self.faces = faces
