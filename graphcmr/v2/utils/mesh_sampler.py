...
class MeshSampler(object):
    ...
    def __init__(self, vertices, faces, sampling_params_file,
                 num_downsampling=1, nsize=1, device=torch.device('cuda')):
        self._A, self._U, self._D = get_graph_params(filename=sampling_params_file, nsize=nsize)
        ...
        ref_vertices = torch.from_numpy(vertices).float()
        ...
        self.faces = torch.from_numpy(faces).int().to(device)
    ...
    def save_mesh(self, filename, vertices=None):
        if vertices == None:
            vertices = self.ref_vertices
        mesh = trimesh.Trimesh(vertices.cpu().numpy(), self.faces.cpu().numpy())
